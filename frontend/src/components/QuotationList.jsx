...
import { FaEdit, FaTrash, FaSync, FaFileInvoice } from 'react-icons/fa';
...
function QuotationList() {
  ...
  const updateStatus = async (id, status) => {
    try {
      const updatedQuotation = await axios.put(`https://possystem-eo7h.onrender.com/api/quotations/${id}`, { status });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setQuotations(quotations.map(q => q._id === _id ? { ...editQuotation } : q));
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Quotation Details', 13, 10);
    autoTable(doc, {
  ...
  const exportToExcel = () => {
    const csv = [
      ['Quotation No', 'Customer Name', 'Total (LKR)', 'Date', 'Status'],
    ...
    a.href = url;
    a.download = 'quotation_details.csv';
    a.click();
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-gradient-to-br from-gray-50 to-white min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-gray-800">Quotation List</h2>
              ...
              <tr key={quotation._id} className="hover:bg-gray-50 transition duration-200">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                  <select
                    ...
                    onChange={(e) => updateStatus(quotation._id, e.target.value)}
                ...
                <td className="px-6 py-4 text-center whitespace-nowrap">
                  <div className="flex justify-center space-x-2">
                    ...
                    {quotation.status === 'complete' && (
                      <button
                        onClick={() => navigate(`/invoice/add?quotationId=${quotation._id}`)}
                        className="bg-teal-500 text-white px-3 py-1 rounded-full hover:bg-teal-600 transition duration-300 flex items-center justify-center"
