...
function ProductForm() {
  ...
  useEffect(() => {
    ...
    if (supplierObjectId) {
      const fetchSupplier = async () => {
        setLoading(true);
        try {
          const response = await axios.get(`https://possystem-eo7h.onrender.com/api/suppliers/${supplierObjectId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await axios.post('https://possystem-eo7h.onrender.com/api/products', formData);
      navigate('/products');
    } catch (err) {
